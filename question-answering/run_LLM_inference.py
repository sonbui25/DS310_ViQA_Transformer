...
Quy tắc bắt buộc:
1. Câu trả lời phải là một đoạn văn bản (span) được lấy NGUYÊN VĂN từ "Đoạn văn". Không được thay đổi từ ngữ, hay thêm từ ngữ.
2. Nếu thông tin không có trong "Đoạn văn", hãy trả về chuỗi rỗng "" (không viết gì cả).
3. Phần văn bản được trích ra thành câu trả lời phải chính xác như được hiển thị trong đoạn văn về format và độ dài. Không thêm bất kỳ từ ngữ nào khác.
    ...
    with tqdm(total=len(all_samples)) as pbar:
        for i in range(0, len(all_samples), args.batch_size):
            ...
            for sample, raw_text in zip(batch_samples, decoded_sequences):
                ...
                # 3. Xóa sạch các thẻ <|...|> (Vệ sinh thẻ lạ)
                clean_answer = re.sub(r"<\|.*?\|>", "", clean_answer)

                # 4. Xử lý xuống dòng & khoảng trắng
                clean_answer = clean_answer.strip()
                
                # Mẹo: Thường câu trả lời đúng chỉ nằm ở dòng đầu tiên sau khi tách
                if '\n' in clean_answer:
                    clean_answer = clean_answer.split('\n')[0]
                
                # 5. FIX LỖI "SYSTEM" & Rỗng
                if clean_answer.lower().strip() == "system":
                    clean_answer = ""

                # 6. Loại bỏ prefix rác
